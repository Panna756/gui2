...
namespace gui2
{
    public partial class ControlForm : Form
    {
        ...
        {
            // เพิ่มเอฟเฟกต์เมื่อชี้เมาส์

            startBtn.BackColor = Color.Transparent;
            startBtn.Paint += (s, e) =>
            {
                using (LinearGradientBrush brush = new LinearGradientBrush(
                    startBtn.ClientRectangle,
                    Color.MediumPurple,
                    Color.LightBlue,
                    LinearGradientMode.Horizontal))
                {
                    e.Graphics.FillRectangle(brush, startBtn.ClientRectangle);
                }
            };

            startBtn.MouseEnter += (s, ev) =>
            {
                startBtn.BackColor = Color.LightGray;
            };

            startBtn.MouseLeave += (s, ev) =>
            {
                startBtn.BackColor = Color.White;
            };

            stopBtn.MouseEnter += (s, ev) =>
            {
                stopBtn.BackColor = Color.LightGray;
            };

            stopBtn.MouseLeave += (s, ev) =>
            {
                stopBtn.BackColor = Color.White;
            };

            readyBtn.MouseEnter += (s, ev) =>
            {
                readyBtn.BackColor = Color.LightGray;
            };

            readyBtn.MouseLeave += (s, ev) =>
            {
                readyBtn.BackColor = Color.White;
            };
            middleBtn.MouseEnter += (s, ev) =>
            {
                middleBtn.BackColor = Color.LightGray;
            };

            middleBtn.MouseLeave += (s, ev) =>
            {
